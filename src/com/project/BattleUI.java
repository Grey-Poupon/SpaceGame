		...
		if (tooltipMenuSelection==null || !tooltipMenuSelection.equals(room)){ // only do stuff if the selcetion has changed
			...
			if(room.equals("q")){// weapons selected
				...
				for(int i=0;i<weapons.length;i++){
					...
					texts[i] = new Text(currentWeapon.getWeaponInfo(),true,Main.WIDTH-591+10,Main.HEIGHT-309-30+(79*(i+1)),"Sevensegies", Font.PLAIN, 50,Color.BLACK);
					newButtons.add(new Button(Main.WIDTH-591,Main.HEIGHT-309+(78*(i)),591,75,ButtonID.battleWeapons[i],bs));
				}
			}
			else if(room.equals("w")) {
				for(int i = 0;i<4;i++) {
					texts[i] = new Text("Engine Choice "+i,true,Main.WIDTH-591+10,Main.HEIGHT-309-30+(78*(i+1)),"Sevensegies", Font.PLAIN, 50,Color.BLACK);
