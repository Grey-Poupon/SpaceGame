...
public class MoleBitch extends Crew {
	...
	public MoleBitch(int social, int combat, int pilot, int engineering,int gunner,int science, int stress, int hunger,
		...
		super(social, combat, pilot, engineering, gunner, science, stress,hunger, gender,RaceID.moleBitch,visible,health);
		generateRaceTable();
		this.setName(names[rand.nextInt(names.length)]);
	}
	public MoleBitch(boolean random,boolean visible,int health) {
		super(getRandomStat(statVariance), getRandomStat(statVariance), getRandomStat(statVariance), getRandomStat(statVariance), getRandomStat(statVariance), getRandomStat(statVariance), 0, 0, getRandomGender(), RaceID.moleBitch, visible,health);
		generateRaceTable();
		this.setName(names[rand.nextInt(names.length)]);
		
	}
	public MoleBitch(boolean visible,int health) {
		super(getRandomWeightedStat(statVariance,(byte)35),getRandomWeightedStat(statVariance,(byte)30), getRandomWeightedStat(statVariance,(byte)5), getRandomWeightedStat(statVariance,(byte)40), getRandomWeightedStat(statVariance,(byte)30), getRandomWeightedStat(statVariance,(byte)30), 0, 0,getRandomGender(), RaceID.moleBitch,visible,health);
		generateRaceTable();
		this.setName(names[rand.nextInt(names.length)]);
	}
